...
因为mock代码比较容易，所以基础代码优化后就能完成。但是生成环境根本不对，需要从头做优化。

### 生产环境下生成代码的方法优化

1. 不能依靠LLM一次生成代码，所以我把生成步骤改为了多次。
2. 生成代码的质量受LLM影响很大，所以我使用了手动提示词

** 生成的代码仍然只是部分有用，这是因为生成的代码的质量不够高。生成代码质量取决于两点，第一是爬虫数据的质量，第二是LLM本身的能力，我现在已经使用了手动提示词，说明LLM(我用的deepseek)生成代码的质量不够！ **
** 另外重新分析整个需求，其实就是让LLM去解析爬取的API文档，然后封装，我认为最佳实践是优化爬虫和提示词，然后生成的代码仍然可能不可用，之后就手动修改，比想办法让LLM生成成功的代码要快很多。而且随着之后不断地迭代，最终生成代码的质量会越来越高。 **

### 瓶颈
1. 单个API文档URL很难完整覆盖调用方法，而如何阅读相关的文档则需要更复杂的爬虫和AI agent来实现。无法短期解决。
2. LLM的代码生成能力极度依赖LLM本身的能力，仍然推荐使用claude code，但是我没有claude api，demo只能使用deepseek，所以LLM生成的代码质量可能不够高。